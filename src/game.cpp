...
Game::Game() : rng_(std::random_device{}()) { Initialize(); }

Game::~Game() { SaveConfig(); }
...
std::string Game::GetVersion() const { return version_; }

void Game::SetColorEnabled(bool enabled) { color_enabled_ = enabled; }
...
void Game::ShowTitle() {
  ClearScreen();
  SetColor("yellow");
  std::cout << std::setw(term_width_ / 2) << std::left << "StarshipAscension v"
            << version_ << std::endl;
  std::cout << std::setw(term_width_ / 2) << std::left << "Copyright 2025"
            << std::endl;
  ResetColor();
...
bool Game::RegisterUser(const std::string &username,
...
bool Game::VerifyUser(const std::string &username,
                      const std::string &password) {
  std::filesystem::path user_path = "data/users/" + username + ".dat";
  if (!std::filesystem::exists(user_path))
    return false;
...
void Game::MainMenu() {
  ...
  std::vector<std::string> options = {
      "New Game", "Resume Game", "Load Game",     "Save Game",
      "Settings", "Help",        "Captain's Log", "Quit"};
...
void Game::NewGame() {
  ...
  ShowProgressBar(
      {{"Generating stars...",
       ...
       {"Generating enemies...", [this]() { enemies_ = {{10, 10}, {20, 20}}; }},
       {"Generating allies...", [this]() { friends_ = {{5, 5}}; }},
       {"Placing starbases...", [this]() { starbases_ = {{25, 25}}; }},
       {"Finalizing game state...", [this]() { Initialize(); }}});

  current_state_ = State::PLAYING;
}

void Game::Resume() { current_state_ = State::PLAYING; }
...
void Game::Settings() {
  ...
  std::cout << "1. Toggle Color: " << (color_enabled_ ? "On" : "Off")
            << std::endl;
  std::cout << "2. Toggle Sound: " << (sound_enabled_ ? "On" : "Off")
            << std::endl;
...
void Game::PlayGame() {
  ...
  std::cout << "Commands: n - navigate, s - scan (long), c - scan (short), f - "
               "fire, r - raise shields, m - main menu"
            << std::endl;
  char cmd = GetKey();
  if (cmd == 'n')
    Navigate();
...
void Game::Navigate() {
  ...
  if (dir == 'w' || dir == 'A')
    ...
    ++position_.first;
  LogEntry("Navigated to (" + std::to_string(position_.first) + ", " +
...
void Game::ScanLongRange() {
  ...
  for (const auto &e : enemies_)
    std::cout << "Enemy at (" << e.first << ", " << e.second << ")"
              << std::endl;
  for (const auto &f : friends_)
    std::cout << "Friendly at (" << f.first << ", " << f.second << ")"
              << std::endl;
  for (const auto &p : planets_)
    std::cout << "Planet at (" << p.first << ", " << p.second << ")"
              << std::endl;
  for (const auto &s : starbases_)
    std::cout << "Starbase at (" << s.first << ", " << s.second << ")"
              << std::endl;
...
void Game::EnemyTurn() {
  ...
  if (found) {
    SetColor("red");
    std::cout << "Enemy at (" << nearest.first << ", " << nearest.second
              << ") attacks you!" << std::endl;
    ResetColor();
    systems_.health -= 10.0;
    if (systems_.health < 0)
      systems_.health = 0;
    LogEntry("Attacked by enemy at (" + std::to_string(nearest.first) + ", " +
             std::to_string(nearest.second) + ")");
  }
}
...
void Game::ShowProgressBar(
  ...
  for (const auto &task : tasks) {
    ClearScreen();
    std::cout << "Loading...\n\n";
    std::cout << task.first << std::endl << std::endl;
...
void Game::ClearScreen() { std::cout << "\033[2J\033[1;1H\n"; }
